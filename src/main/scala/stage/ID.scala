...
class ID extends Component {
  ...
  switch(opcode){
    ...
    is(OPCODE.STI) {DTUSetOp(DTU_OPCODE.STI,REG_READ_STRING.RD_RS1,IMM_TYPE.S_TYPE)}  //有符号扩展，使用长一点的
    is(OPCODE.LDI) {DTUSetOp(DTU_OPCODE.LDI,REG_READ_STRING.RD,IMM_TYPE.I_TYPE)}
