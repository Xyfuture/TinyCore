...
class ID extends Component {
  ...
  switch(opcode){
    ...
    is(OPCODE.SREG) { sreg() }
    is(OPCODE.SADDI) {SEUSetOp(SEU_OPCODE.SADD,REG_READ_STRING.RD_RS1,IMM_TYPE.S_TYPE)}
    is(OPCODE.SSUBI) {SEUSetOp(SEU_OPCODE.SSUB,REG_READ_STRING.RD_RS1,IMM_TYPE.S_TYPE)}
    is(OPCODE.SMULI) {SEUSetOp(SEU_OPCODE.SMUL,REG_READ_STRING.RD_RS1,IMM_TYPE.S_TYPE)}
